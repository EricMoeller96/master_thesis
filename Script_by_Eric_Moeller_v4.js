...
var month_start = 6;                    //Startmonth of the temporal integration
var month_end = 8;                      //Endmonth of the temporal integration

var year_start = 2020;                  //Startyear of the temporal integration.
var year_end = 2020;                    //Endyear of the temporal integration.

var Date_start = '2020-06-01';          //'YYYY-MM-DD'; Care about the starting year of the satellite
var Date_end = '2020-08-31';            //'YYYY-MM-DD'

var max_cloud_cover = 20;               //Maximum Cloud Cover (on land) for Landsat & Sentinel between 0 and 100
...
var percentile_value = [75];            //This variable will be activated when var metric = 'percentile'; Value must be between 0 and 100
...
var month_start_Postevent = 6;          //Startmonth of the temporal integration (Postevent)  
var month_end_Postevent = 7;           //Endyear of the temporal integration. (Postevent)

var year_start_Postevent = 2019;        //Startyear of the temporal integration. (Postevent)
var year_end_Postevent = 2020;          //Endyear of the temporal integration. (Postevent)

var Date_start_Postevent = '2019-06-01';  //'YYYY-MM-DD'
var Date_end_Postevent = '2019-07-31';    //'YYYY-MM-DD'
//======================================================================================

//Export single images to google drive
var export_singleimage = 'No';          // 'Yes' or 'No'
var imageID = 0;                        // Console: All images (with indices) --> Select one 
                                        //image and get the number
                                          
var region = 'Wuerzburg';                  //ROI; Just for Exporting Data Name
var time_period = 'Summer2020';                              //Just for Exporting Data Name
//============================================================================================
...
// Define functions to select and rename bands for Landsat-8 without pixel_qa
...
var Landsat_median_RGB_Params = {bands: ['R', 'G', 'B'], min: 0, max: 4000, gamma: 1.5};   
var Sentinel_median_RGB_Params = {bands: ['R', 'G', 'B'], min:0, max:2000, gamma: 1.5};
...
// Merge all collections
  var Merge_col_Landsat = L4_col.merge(L5_col).merge(L7_col).merge(L8_col);
// Sort by date
...
//=====IF DIFFERENCED INDICES=================================================================
  ...
  if (Differenced == 'Yes') {
    ...
    var L4_col_POSTEVENT = ee.ImageCollection('LANDSAT/LT04/C01/T1_SR')
    .filterBounds(geometry)
    .filter(ee.Filter.lt('CLOUD_COVER_LAND', max_cloud_cover))
    .map(renameBandsTM_ETM)
    .map(cloudMaskLandsat)
    ...
    var L5_col_POSTEVENT = ee.ImageCollection('LANDSAT/LT05/C01/T1_SR')
    .filterBounds(geometry)
    .filter(ee.Filter.lt('CLOUD_COVER_LAND', max_cloud_cover))
    .map(renameBandsTM_ETM)
    .map(cloudMaskLandsat)
    ...
    var L7_col_POSTEVENT = ee.ImageCollection('LANDSAT/LE07/C01/T1_SR')
    .filterBounds(geometry)
    .filter(ee.Filter.lt('CLOUD_COVER_LAND', max_cloud_cover))
    .map(renameBandsTM_ETM)
    .map(cloudMaskLandsat)
    ...
    var L8_col_POSTEVENT = ee.ImageCollection('LANDSAT/LC08/C01/T1_SR')
    .filterBounds(geometry)
    .filter(ee.Filter.lt('CLOUD_COVER_LAND', max_cloud_cover))
    .map(renameBandsOLI)
    .map(cloudMaskLandsat)
    ...
    // Merge collections
    var Merge_col_Landsat_Postevent = L4_col_POSTEVENT.merge(L5_col_POSTEVENT).merge(L7_col_POSTEVENT).merge(L8_col_POSTEVENT);
    // Sort by date
...
if (export_to_drive == 'Yes' && metric != 'percentile' && Differenced != 'Yes') {
  Export.image.toDrive({
  image: temp.reduce(metric),
  description: pl + '_All_bands_and_indices_'+ metric + '_' + Date_start + '_' + Date_end + '_'
...
if (export_to_drive == 'Yes' && metric == 'percentile' && Differenced != 'Yes') {
  Export.image.toDrive({
  image: temp.reduce(ee.Reducer.percentile(percentile_value)),
  description: pl + '_All_bands_and_indices_'+ metric + '_' + percentile_value + '_' + Date_start + '_' + Date_end + '_'
  + region + '_' + time_period,
  folder: 'GEE_Export',
  scale: scale,
  crs: epsg,
...
if (export_to_drive == 'Yes' && Differenced == 'Yes') {
  ...
  description: pl + '_differenced_' + index + '_' + metric + '_' + Date_start + '_' + Date_end + '-'
...
if (satellite == 'Landsat' && metric != 'percentile'){
  if (index == 'NDVI') {
...
else if (satellite == 'Sentinel' && metric != 'percentile'){
...
else if (satellite == 'Modis' && metric != 'percentile'){
